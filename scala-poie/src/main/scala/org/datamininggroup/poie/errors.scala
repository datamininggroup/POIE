...
package errors {
  ...
  /** Exception found in the syntax of the PFA document (valid JSON but invalid structure for PFA).
    * 
    * A `PFASyntaxException` is raised if the rules in the PFA specification are violated to the extent that poie cannot build an abstract syntax tree.
    * 
    * @param message human-readable message
  ...
  /** Exception found in the semantics of the PFA document (type mismatches, unrecognized symbols, etc.).
    * 
    * A `PFASemanticException` is raised if the rules in the PFA specification are violated to the extent that poie cannot build Java code from the abstract syntax tree.
    * 
    * @param message human-readable message
    * @param pos locator mark, PFA path index, or `None`
    */
  ...
  /** Exception found in the initialization of the PFA document (loading cell/pool data, initializing scoring engine state).
    * 
    * A `PFAInitializationException` is raised if the rules in the PFA specification are violated to the extent that poie cannot provide a scoring engine instance to the calling program.
    * 
    * @param message human-readable message.
    */
