...
package options {
  /** Represents the `options` section of a PFA document with host-specific overrides.
    ...
    * 
    * Recognized option names whose option type is incorrect raise [[org.datamininggroup.poie.errors.PFAInitializationException PFAInitializationException]].
    * 
    * Options currently supported by poie:
    * 
    *  - "timeout" (`Long`): number of milliseconds to allow a PFA method to run before raising [[org.datamininggroup.poie.errors.PFATimeoutException PFATimeoutException]].
