...
import org.datamininggroup.poie.datatype.AvroConversions._
...
import org.datamininggroup.poie.datatype.AvroPlaceholder
...
import org.datamininggroup.poie.reader.JsonDom
import org.datamininggroup.poie.reader.JsonObject
import org.datamininggroup.poie.reader.JsonArray
import org.datamininggroup.poie.reader.JsonNull
import org.datamininggroup.poie.reader.JsonTrue
import org.datamininggroup.poie.reader.JsonFalse
import org.datamininggroup.poie.reader.JsonString
import org.datamininggroup.poie.reader.JsonLong
import org.datamininggroup.poie.reader.JsonDouble
...
package ast {
  ...
  /** Represents a user-defined function.
    ...
    * @param sig function signature (note that [[org.datamininggroup.poie.signature.Sigs Sigs]] is not allowed for user-defined functions)
    */
  ...
  case class EmitFcn(outputType: AvroType) extends Fcn {
    ...
    def javaCode(args: Seq[JavaCode], argContext: Seq[AstContext], paramTypes: Seq[Type], retType: AvroType, engineOptions: EngineOptions): JavaCode =
      JavaCode("org.datamininggroup.poie.jvmcompiler.W.n(f_emit.apply(%s))", wrapArg(0, args, paramTypes, true).toString)
  }
  ...
  /** Trait for [[org.datamininggroup.poie.ast.Ast Ast]] context classes.
    */
  trait AstContext
  /** Subtrait for context classes of [[org.datamininggroup.poie.ast.Argument Argument]].
    */
  trait ArgumentContext extends AstContext {
    val calls: Set[String]
  }
  /** Subtrait for context classes of [[org.datamininggroup.poie.ast.Expression Expression]].
    */
  trait ExpressionContext extends ArgumentContext {
    val retType: AvroType
  }
  /** Subtrait for context classes of [[org.datamininggroup.poie.ast.FcnDef FcnDef]].
    */
  trait FcnContext extends ArgumentContext {
    val fcnType: FcnType
  }
  /** Trait for result of a generic task, passed to [[org.datamininggroup.poie.ast.Ast Ast]] `walk`.
    */
  trait TaskResult

  /** Trait for a generic task, passed to [[org.datamininggroup.poie.ast.Ast Ast]] `walk`.
    */
  trait Task extends Function3[AstContext, EngineOptions, Option[Type], TaskResult] {
    /** Perform a task from context generated by a [[org.datamininggroup.poie.ast.Ast Ast]] `walk`.
      * 
      * @param astContext data about the [[org.datamininggroup.poie.ast.Ast Ast]] node after type-checking
  ...
  /** Concrete [[org.datamininggroup.poie.ast.Task Task]] that does nothing, used for type-checking without producing an engine.
    */
  object NoTask extends Task {
    /** Concrete [[org.datamininggroup.poie.ast.TaskResult TaskResult]] that contains no result.
      */
  ...
  trait Ast {
    ...
    /** Walk over tree applying a partial function, returning a list of results in its domain.
      * 
      * @param pf partial function that takes any [[org.datamininggroup.poie.ast.Ast Ast]] as an argument, returning anything
    ...
    /** Walk over tree applying a partial function, returning a transformed copy of the tree.
      * 
      * @param pf partial function that takes any [[org.datamininggroup.poie.ast.Ast Ast]] as an argument, returning a replacement [[org.datamininggroup.poie.ast.Ast Ast]]
    ...
    /** Walk over tree applying a [[org.datamininggroup.poie.ast.Task Task]] while checking for semantic errors.
      * 
      * This is how Java is generated from an abstract syntax tree: the [[org.datamininggroup.poie.ast.Task Task]] in that case is [[org.datamininggroup.poie.jvmcompiler.JVMCompiler JVMCompiler]].
      * 
  ...
  /** Interpreter for cell or pool data in a JSON file outside of the original PFA document.
    * 
    * Streams the external JSON data directly into live objects, skipping the [[org.datamininggroup.poie.reader.JsonDom JsonDom]] intermediary.
    */
  ...
  /** Abstract syntax tree for a `cell` definition.
    ...
    * @param shared if `true`, this cell shares data with all others in the same [[org.datamininggroup.poie.shared.SharedState SharedState]]
  ...
  /** Abstract syntax tree for a `pool` definition.
    ...
    * @param shared if `true`, this pool shares data with all others in the same [[org.datamininggroup.poie.shared.SharedState SharedState]]
  ...
  /** Trait for path index elements, which can be used in `attr`, `cell`, and `pool` `path` arrays.
    */
  trait PathIndex
  /** Array indexes, which are concrete [[org.datamininggroup.poie.ast.PathIndex PathIndex]] elements that dereference arrays (expressions of `int` type).
    */
  case class ArrayIndex(i: TaskResult, t: AvroType) extends PathIndex
  /** Map indexes, which are concrete [[org.datamininggroup.poie.ast.PathIndex PathIndex]] elements that dereference maps (expressions of `string` type).
    */
  case class MapIndex(k: TaskResult, t: AvroType) extends PathIndex
  /** Record indexes, which are concrete [[org.datamininggroup.poie.ast.PathIndex PathIndex]] elements that dereference records (literal `string` expressions).
    */
  case class RecordIndex(f: String, t: AvroType) extends PathIndex

  /** Mixin for [[org.datamininggroup.poie.ast.Ast Ast]] classes taht have paths (`attr`, `cell`, `pool`).
    */
  trait HasPath {
  ...
  case class FcnDef(paramsPlaceholder: Seq[(String, AvroPlaceholder)], retPlaceholder: AvroPlaceholder, body: Seq[Expression], pos: Option[String] = None) extends Argument {
    ...
    /** Resolved parameter types (list of string -> [[org.datamininggroup.poie.datatype.AvroType AvroType]] singletons).
      */
  ...
  object BinaryFormatter {
    ...
    /** Convert a format string to a [[org.datamininggroup.poie.ast.BinaryFormatter.Declare Declare]] object.
      * 
