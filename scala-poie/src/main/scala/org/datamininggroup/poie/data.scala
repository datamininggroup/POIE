...
import org.datamininggroup.poie.shared.PathIndex
import org.datamininggroup.poie.shared.I
import org.datamininggroup.poie.shared.M
import org.datamininggroup.poie.shared.R
...
package data {
  ...
  /** Overrides Avro's data model with one designed for PFA.
    ...
    *  - "array" are loaded into [[org.datamininggroup.poie.data.PFAArray PFAArray]]
    *  - "map" are loaded into [[org.datamininggroup.poie.data.PFAMap PFAMap]]
    *  - "fixed" are loaded into [[org.datamininggroup.poie.data.GenericPFAFixed GenericPFAFixed]]
    *  - "enum" are loaded into [[org.datamininggroup.poie.data.GenericPFAEnumSymbol GenericPFAEnumSymbol]]
    *  - "record" are loaded into [[org.datamininggroup.poie.data.GenericPFARecord GenericPFARecord]]
    * 
  ...
  /** Overrides Avro's data model with one designed for PFA with classes customized for each PFA engine.
    ...
    *  - "array" are loaded into [[org.datamininggroup.poie.data.PFAArray PFAArray]]
    *  - "map" are loaded into [[org.datamininggroup.poie.data.PFAMap PFAMap]]
    *  - "fixed" are loaded into a dynamically generated subclass of [[org.datamininggroup.poie.data.PFAFixed PFAFixed]]
    *  - "enum" are loaded into a dynamically generated subclass of [[org.datamininggroup.poie.data.PFAEnumSymbol PFAEnumSymbol]]
    *  - "record" are loaded into a dynamically generated subclass of [[org.datamininggroup.poie.data.PFARecord PFARecord]]
    * 
  ...
  /** Interface for [[org.datamininggroup.poie.data.GenericPFARecord GenericPFARecord]] all dynamically generated subclasses of [[org.datamininggroup.poie.data.PFARecord PFARecord]].
    */
  trait AnyPFARecord {
  ...
  /** Interface for [[org.datamininggroup.poie.data.GenericPFAFixed GenericPFAFixed]] all dynamically generated subclasses of [[org.datamininggroup.poie.data.PFAFixed PFAFixed]].
    */
  trait AnyPFAFixed {
    /** Access to schema.
      */
  ...
  /** Interface for [[org.datamininggroup.poie.data.GenericPFAEnumSymbol GenericPFAEnumSymbol]] all dynamically generated subclasses of [[org.datamininggroup.poie.data.PFAEnumSymbol PFAEnumSymbol]].
    */
  ...
  /** Translates data created by one [[org.datamininggroup.poie.jvmcompiler.PFAEngine PFAEngine]] into a form usable by another [[org.datamininggroup.poie.jvmcompiler.PFAEngine PFAEngine]].
    * 
  ...
  /** Translates data created by the Avro library (generic or specific) into a form usable by a [[org.datamininggroup.poie.jvmcompiler.PFAEngine PFAEngine]].
    * 
  ...
  /** Translates Scala data into a form usable by a [[org.datamininggroup.poie.jvmcompiler.PFAEngine PFAEngine]].
    * 
    *  - Any Scala `Seq` is converted to a [[org.datamininggroup.poie.data.PFAArray PFAArray]].
    *  - Any Scala `Map` is converted to a [[org.datamininggroup.poie.data.PFAMap PFAMap]].
    *  - Any Scala case class is converted to a [[org.datamininggroup.poie.data.PFARecord PFARecord]].
    * 
    * @param avroType type of the datum
    * @param classLoader `ClassLoader` of the destination engine
    */
...
package object data {
  ...
  /** Convert data from JSON to a generic PFA object.
    * 
    * Needs to be translated to a specific engine with a [[org.datamininggroup.poie.data.PFADataTranslator PFADataTranslator]] or the engine's `fromPFAData` method.
    * 
  ...
  /** Create an Avro iterator (subclass of `java.util.Iterator`) over Avro-serialized input data.
    * 
    * The objects produced by this iterator need to be translated to a specific engine with a [[org.datamininggroup.poie.data.PFADataTranslator PFADataTranslator]] or the engine's `fromPFAData` method.
    * 
  ...
  /** Create an iterator over CSV-serialized input data.
    * 
    * The objects produced by this iterator are need to be translated to a specific engine with a [[org.datamininggroup.poie.data.PFADataTranslator PFADataTranslator]] or the engine's `fromPFAData` method.
    * 
  ...
  /** Convert a PFA object into an in-house [[org.datamininggroup.poie.reader.JsonDom JsonDom]].
    * 
