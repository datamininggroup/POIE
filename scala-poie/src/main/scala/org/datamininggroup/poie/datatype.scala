...
import org.datamininggroup.poie.util.uniqueEnumName
import org.datamininggroup.poie.util.uniqueFixedName
import org.datamininggroup.poie.util.uniqueRecordName
...
package datatype {
  ...
  trait Type {
    ...
    /** Return `this` if an [[org.datamininggroup.poie.datatype.AvroType AvroType]], raise an `IllegalArgumentException` otherwise.
      */
  ...
  /** Import this object to get implicit conversions among `org.apache.avro.Schema`, [[org.datamininggroup.poie.datatype.AvroType AvroType]], [[org.datamininggroup.poie.datatype.AvroPlaceholder AvroPlaceholder]], and their JSON representation.
    */
  object AvroConversions {
    /** Convert a `org.apache.avro.Schema` from the Avro library into a poie [[org.datamininggroup.poie.datatype.AvroType AvroType]].
      */
    ...
    /** Convert a `org.apache.avro.Schema` from the Avro library into a poie [[org.datamininggroup.poie.datatype.AvroPlaceholder AvroPlaceholder]].
      */
    implicit def schemaToAvroPlaceholder(schema: Schema): AvroPlaceholder = AvroFilledPlaceholder(schemaToAvroType(schema))
    /** Convert a poie [[org.datamininggroup.poie.datatype.AvroType AvroType]] into a `org.apache.avro.Schema` for the Avro library.
      */
    ...
    /** Convert a file containing an Avro schema in JSON (usually ending with ".avsc") into a poie [[org.datamininggroup.poie.datatype.AvroType AvroType]].
      */
    implicit def fileToAvroType(x: java.io.File): AvroType = schemaToAvroType((new Schema.Parser).parse(x))
    /** Convert an `InputStream` containing an Avro schema in JSON into a poie [[org.datamininggroup.poie.datatype.AvroType AvroType]].
      */
    implicit def inputStreamToAvroType(x: java.io.InputStream): AvroType = schemaToAvroType((new Schema.Parser).parse(x))
    /** Convert a `String` containing an Avro schema in JSON into a poie [[org.datamininggroup.poie.datatype.AvroType AvroType]].
      */
  ...
  /** Pseudo-type for exceptions (the "bottom" type in type theory).
    */
  private[poie] case class ExceptionType() extends AvroType {
    val name = "exception"
  ...
  class AvroPlaceholder(original: String, forwardDeclarationParser: ForwardDeclarationParser) {
    ...
    /** Called after [[org.datamininggroup.poie.datatype.AvroTypeBuilder AvroTypeBuilder]] `resolveTypes` to get the resolved type.
      */
    ...
    /** The [[org.datamininggroup.poie.datatype.ForwardDeclarationParser ForwardDeclarationParser]] responsible for this placeholder.
      */
  ...
  /** Used to create [[org.datamininggroup.poie.datatype.AvroPlaceholder AvroPlaceholder]] objects to satisfy functions that require them, yet the type is already known.
    */
  ...
  /** Container that stores Avro types as they're collected from a PFA file, returning [[org.datamininggroup.poie.datatype.AvroPlaceholder AvroPlaceholder]] objects, and then resolves those types indepenedent of the order in which they were read from the file.
    */
  ...
  /** Factory that coordinates the process of collecting Avro strings, putting them in a [[org.datamininggroup.poie.datatype.ForwardDeclarationParser ForwardDeclarationParser]], and then resolving them all at the end, independent of order.
    */
