...
import org.datamininggroup.poie.lib.math.Round
...
package object core {
  ...
  class Comparison(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.datamininggroup.poie.data.NumericalComparison")
      case x: AvroType =>
        JavaCode("(new org.datamininggroup.poie.data.Comparison(%s))", javaSchema(x, false))
      case _ =>
  ...
  class Equal(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.datamininggroup.poie.data.NumericalEQ")
      case x: AvroType =>
        JavaCode("(new org.datamininggroup.poie.data.ComparisonOperator(%s, 1))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
  }
  ...
  class GreaterOrEqual(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.datamininggroup.poie.data.NumericalGE")
      case x: AvroType =>
        JavaCode("(new org.datamininggroup.poie.data.ComparisonOperator(%s, 2))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
  }
  ...
  class GreaterThan(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.datamininggroup.poie.data.NumericalGT")
      case x: AvroType =>
        JavaCode("(new org.datamininggroup.poie.data.ComparisonOperator(%s, 3))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
  }
  ...
  class NotEqual(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.datamininggroup.poie.data.NumericalNE")
      case x: AvroType =>
        JavaCode("(new org.datamininggroup.poie.data.ComparisonOperator(%s, -1))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
  ...
  class LessThan(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.datamininggroup.poie.data.NumericalLT")
      case x: AvroType =>
        JavaCode("(new org.datamininggroup.poie.data.ComparisonOperator(%s, -2))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
  ...
  class LessOrEqual(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.datamininggroup.poie.data.NumericalLE")
      case x: AvroType =>
        JavaCode("(new org.datamininggroup.poie.data.ComparisonOperator(%s, -3))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
  ...
  class Max(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.ret match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble =>
        JavaCode("org.datamininggroup.poie.data.NumericalMax")
      case x: AvroType =>
        JavaCode("(new org.datamininggroup.poie.data.ComparisonMax(%s))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
  ...
  class Min(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.ret match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble =>
        JavaCode("org.datamininggroup.poie.data.NumericalMin")
      case x: AvroType =>
        JavaCode("(new org.datamininggroup.poie.data.ComparisonMin(%s))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
    override def javaCode(args: Seq[JavaCode], argContext: Seq[AstContext], paramTypes: Seq[Type], retType: AvroType, engineOptions: EngineOptions): JavaCode = retType match {
