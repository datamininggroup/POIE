...
import org.datamininggroup.poie.data.NumericalLT
import org.datamininggroup.poie.data.ComparisonOperatorLT
...
package object array {
  ...
  class Sort(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      ...
      case AvroArray(x) =>
        JavaCode("(new " + this.getClass.getName + "$SorterOther(new org.datamininggroup.poie.data.ComparisonOperatorLT(%s)))", javaSchema(x, false))
      case _ =>
  ...
  class Max(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      ...
      case AvroArray(x) =>
        JavaCode("(new " + this.getClass.getName + "$MaxOther(new org.datamininggroup.poie.data.ComparisonOperatorLT(%s), " + posToJava + "))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
  }
  ...
  class Min(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      ...
      case AvroArray(x) =>
        JavaCode("(new " + this.getClass.getName + "$MinOther(new org.datamininggroup.poie.data.ComparisonOperatorLT(%s), " + posToJava + "))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
  }
  ...
  class Median(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      ...
      case AvroArray(x) =>
        JavaCode("(new " + this.getClass.getName + "$SorterOther(new org.datamininggroup.poie.data.ComparisonOperatorLT(%s), " + posToJava + "))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
  }
  ...
  class Mode(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      ...
      case AvroArray(AvroDouble()) =>
        JavaCode("(new " + this.getClass.getName + "$SorterDouble(" + posToJava + "))")
      case AvroArray(x) =>
        JavaCode("(%s)(new %s$SorterOther(new org.datamininggroup.poie.data.ComparisonOperatorLT(%s), " + posToJava + "))", javaType(fcnType.ret, false, true, false), this.getClass.getName, javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
  }
