...
package signature {
  ...
  /** Describes the range of support of a function signature (or, in the future, special form) in terms of an optional beginning of life (birth), and optional deprecation and end of life (death).
    ...
    * 
    * At a given [[org.datamininggroup.poie.signature.PFAVersion PFAVersion PFAVersion]], the Lifespan has three possible states: current (method `current` returns `true`), deprecated (method `deprecated` returns `true`), and non-existent (both `current` and `deprecated` return `false`). Method `current` and `deprecated` are mutually exclusive; for a given [[org.datamininggroup.poie.signature.PFAVersion PFAVersion PFAVersion]], they would never both return `true`.
    */
  ...
  /** Trait for a type pattern.
    * A type pattern is something that a [[org.datamininggroup.poie.datatype.AvroType AvroType AvroType]] is matched against when determining if a PFA function signature can be applied to a given set of arguments.
    * 
    * It could be as simple as the type itself (e.g. `P.Int` matches `AvroInt()`) or it could be a complex wildcard.
    */
  trait Pattern
  object P {
    /** Matches [[org.datamininggroup.poie.datatype.AvroNull AvroNull AvroNull]].
      */
    case object Null extends Pattern
    /** Matches [[org.datamininggroup.poie.datatype.AvroBoolean AvroBoolean AvroBoolean]].
      */
    case object Boolean extends Pattern
    /** Matches [[org.datamininggroup.poie.datatype.AvroInt AvroInt AvroInt]].
      */
    case object Int extends Pattern
    /** Matches [[org.datamininggroup.poie.datatype.AvroLong AvroLong AvroLong]].
      */
    case object Long extends Pattern
    /** Matches [[org.datamininggroup.poie.datatype.AvroFloat AvroFloat AvroFloat]].
      */
    case object Float extends Pattern
    /** Matches [[org.datamininggroup.poie.datatype.AvroDouble AvroDouble AvroDouble]].
      */
    case object Double extends Pattern
    /** Matches [[org.datamininggroup.poie.datatype.AvroBytes AvroBytes AvroBytes]].
      */
    case object Bytes extends Pattern
    /** Matches [[org.datamininggroup.poie.datatype.AvroString AvroString AvroString]].
      */
    case object String extends Pattern

    /** Matches [[org.datamininggroup.poie.datatype.AvroArray AvroArray AvroArray]] with a given `items` pattern.
      * 
      * @param items the item pattern to match
      */
    case class Array(items: Pattern) extends Pattern
    /** Matches [[org.datamininggroup.poie.datatype.AvroMap AvroMap AvroMap]] with a given `values` pattern.
      * 
      * @param values the values pattern to match
      */
    case class Map(values: Pattern) extends Pattern
    /** Matches [[org.datamininggroup.poie.datatype.AvroUnion AvroUnion AvroUnion]] with a given set of sub-patterns.
      * 
      * @param types patterns for the subtypes
      */
    case class Union(types: List[Pattern]) extends Pattern

    /** Matches [[org.datamininggroup.poie.datatype.AvroFixed AvroFixed AvroFixed]] with a given size and an optional name.
      * 
    ...
    /** Matches [[org.datamininggroup.poie.datatype.AvroEnum AvroEnum AvroEnum]] with given symbols and an optional name.
      * 
      * @param symbols names of the enum symbols
      * @param fullName optional name for the fixed pattern (if not provided, fixed types of any name would match)
      */
    case class Enum(symbols: List[String], fullName: Option[String] = None) extends Pattern
    /** Matches [[org.datamininggroup.poie.datatype.AvroRecord AvroRecord AvroRecord]] with given fields and an optional name.
      * 
    ...
    /** Matches [[org.datamininggroup.poie.datatype.FcnType FcnType FcnType]] with a given sequence of parameter patterns and return pattern.
      * 
    ...
    /** Matches any [[org.datamininggroup.poie.datatype.AvroType AvroType AvroType]] or one of a restricted set.
      * 
    ...
    /** Matches a [[org.datamininggroup.poie.datatype.AvroRecord AvroRecord AvroRecord]] with '''at least''' the requested set of fields.
      * 
      * Label letters are shared across a signature (e.g. if two wildcards are both labeled "A", then they both have to resolve to the same type).
      * 
      * @param label label letter (usually one character long, but in principle an arbitrary string)
    ...
    /** Matches a [[org.datamininggroup.poie.datatype.AvroEnum AvroEnum AvroEnum]] without any constraint on the symbol names.
      * 
      * Label letters are shared across a signature (e.g. if two wildcards are both labeled "A", then they both have to resolve to the same type).
      * 
      * @param label label letter (usually one character long, but in principle an arbitrary string)
    ...
    /** Matches a [[org.datamininggroup.poie.datatype.AvroFixed AvroFixed AvroFixed]] without any constraint on the size.
      * 
      * Label letters are shared across a signature (e.g. if two wildcards are both labeled "A", then they both have to resolve to the same type).
      * 
      * @param label label letter (usually one character long, but in principle an arbitrary string)
    ...
    /** Matches a [[org.datamininggroup.poie.datatype.AvroEnum AvroEnum AvroEnum]] whose symbols match the fields of a given record
      * 
    ...
    /** Convert a patternt to a type, if possible (wildcards can't be converted to types).
      ...
      * @return corresponding type ([[org.datamininggroup.poie.datatype.Type Type]] rather than [[org.datamininggroup.poie.datatype.AvroType AvroType]] to allow for [[org.datamininggroup.poie.datatype.FcnType FcnType]])
      */
    ...
    /** Raise [[org.datamininggroup.poie.signature.IncompatibleTypes IncompatibleTypes]] if a given [[org.datamininggroup.poie.datatype.Type Type]] is not a [[org.datamininggroup.poie.datatype.AvroType AvroType]]; otherwise, pass through.
      * 
  ...
  /** PFA function signature for ad-hoc polymorphism (list of different signatures supported by the function).
    * 
    * @param cases signatures supported by this function; the order of this list is the order in which poie will attempt to resolve the signatures (the first one that matches is used)
    */
  ...
  /** PFA function signature for a single pattern.
    * 
    * This class can either be used directly as a function's only signature or it may be contained within a [[org.datamininggroup.poie.signature.Sigs Sigs]].
    * 
