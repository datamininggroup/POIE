...
import poie.pfaast
import poie.datatype
import poie.fcn
import poie.options
import poie.P as P
import poie.reader
import poie.signature
import poie.util
from poie.util import DynamicScope
import poie.version

from poie.pfaast import EngineConfig
from poie.pfaast import Cell as AstCell
from poie.pfaast import Pool as AstPool
from poie.pfaast import FcnDef
from poie.pfaast import FcnRef
from poie.pfaast import FcnRefFill
from poie.pfaast import CallUserFcn
from poie.pfaast import Call
from poie.pfaast import Ref
from poie.pfaast import LiteralNull
from poie.pfaast import LiteralBoolean
from poie.pfaast import LiteralInt
from poie.pfaast import LiteralLong
from poie.pfaast import LiteralFloat
from poie.pfaast import LiteralDouble
from poie.pfaast import LiteralString
from poie.pfaast import LiteralBase64
from poie.pfaast import Literal
from poie.pfaast import NewObject
from poie.pfaast import NewArray
from poie.pfaast import Do
from poie.pfaast import Let
from poie.pfaast import SetVar
from poie.pfaast import AttrGet
from poie.pfaast import AttrTo
from poie.pfaast import CellGet
from poie.pfaast import CellTo
from poie.pfaast import PoolGet
from poie.pfaast import PoolTo
from poie.pfaast import PoolDel
from poie.pfaast import If
from poie.pfaast import Cond
from poie.pfaast import While
from poie.pfaast import DoUntil
from poie.pfaast import For
from poie.pfaast import Foreach
from poie.pfaast import Forkeyval
from poie.pfaast import CastCase
from poie.pfaast import CastBlock
from poie.pfaast import Upcast
from poie.pfaast import IfNotNull
from poie.pfaast import BinaryFormatter
from poie.pfaast import Pack
from poie.pfaast import Unpack
from poie.pfaast import Doc
from poie.pfaast import Error
from poie.pfaast import Try
from poie.pfaast import Log

from poie.pfaast import Method
from poie.pfaast import ArrayIndex
from poie.pfaast import MapIndex
from poie.pfaast import RecordIndex

from poie.pmml.reader import pmmlToAst

class GeneratePython(poie.pfaast.Task):
    """A ``poie.pfaast.Task`` for turning PFA into executable Python."""

    @staticmethod
    def makeTask(style):
        """Make a ``poie.genpy.GeneratePython`` Task with a particular style.

        Currently, the only style is "pure" (``poie.genpy.GeneratePythonPure``).
        """
    ...
    def __call__(self, context, engineOptions):
        ...
        if isinstance(context, EngineConfig.Context):
            if context.name is None:
                name = poie.util.uniqueEngineName()
            else:
                name = context.name
...
""")
            ...
            return repr(poie.datatype.jsonDecoder(context.retType, json.loads(context.value)))
            ...
            if isinstance(context.retType, poie.datatype.AvroUnion) and not isinstance(context.originalType, poie.datatype.AvroUnion):
...
class GeneratePythonPure(GeneratePython):
    """A ``poie.pfaast.Task`` for generating a pure Python executable.

    This is a dummy class; all of the work is done in ``poie.genpy.GeneratePython``. Non-pure styles would be siblings of this class.
    """
    pass
...
def update(state, scope, obj, path, to, arrayErrCode, mapErrCode, fcnName, pos):
    ...
    :type state: pypoie.genpy.ExecutionState
    :param state: runtime state object
    :type scope: pypoie.util.DynamicScope
...
def cast(state, scope, expr, fromType, cases, partial, parser):
    ...
    :type parser: pypoie.datatype.ForwardDeclarationParser
    ...
    for name, toType, clause in cases:
        ...
        if isinstance(fromType, poie.datatype.AvroUnion) and isinstance(expr, dict) and len(expr) == 1:
            tag, = list(expr.keys())
        ...
        try:
            castValue = poie.datatype.jsonDecoder(toType, value)
...
def unpackOne(bytes, scope, s, f, l):
    ...
    if f == "raw":
        ...
        try:
            nullbyte = bytes.index(chr(0).encode())
        except ValueError:
            raise MisalignedPacking()
        else:
            this = bytes[:nullbyte]
...
def unpack(state, scope, bytes, format, thenClause):
    ...
    :type thenClause: callable
    :param thenClause: function that is called if there was no poie.genpy.MisalignedPacking exception
    :rtype: ``None``
    :return: nothing
    """
    thenScope = DynamicScope(scope)
...
def unpackElse(state, scope, bytes, format, thenClause, elseClause):
    ...
    :param thenClause: function that is called if there was no poie.genpy.MisalignedPacking exception
    :type elseClause: callable
    :param elseClause: function that is called if there was an poie.genpy.MisalignedPacking exception
    :rtype: result of ``thenClause`` or ``elseClause``
    :return: if there was no poie.genpy.MisalignedPacking exception, returns result of ``thenClause``, otherwise, returns result of ``elseClause``
    """
...
def error(message, code, pos):
    ...
    :type message: string
    :param message: message for the poie.errors.PFAUserException
    ...
    :rtype: bottom type!
    :return: never returns; always raises a poie.errors.PFAUserException
    """
...
class FakeEmitForExecution(poie.fcn.Fcn):
...
def checkForDeadlock(engineConfig, engine):
    """Checks a poie.pfaast.EngineConfig for the possibility of deadlock.

    If any function used as an updator eventually calls some other function that would update state, this function raises poie.errors.PFAInitializationException.
    """
...
class PFAEngine(object):
    """Base class for a poie scoring engine.
    ...
    **Data format:**
    ...
    def fromAst(engineConfig, options=None, version=None, sharedState=None, multiplicity=1, style="pure", debug=False):
        """Create a collection of instances of this scoring engine from a PFA abstract syntax tree (``poie.pfaast.EngineConfig``).

        :type engineConfig: pypoie.pfaast.EngineConfig
        :param engineConfig: a parsed, interpreted PFA document, i.e. produced by ``poie.reader.jsonToAst``
        ...
        :type sharedState: pypoie.genpy.SharedState
        ...
        functionTable = poie.pfaast.FunctionTable.blank()

        engineOptions = poie.options.EngineOptions(engineConfig.options, options)
        if version is None:
            version = poie.version.defaultPFAVersion
        pfaVersion = poie.signature.PFAVersion.fromString(version)

        context, code = engineConfig.walk(GeneratePython.makeTask(style), poie.pfaast.SymbolTable.blank(), functionTable, engineOptions, pfaVersion)
        if debug:
            print(code)
        sandbox = {# Scoring engine architecture
                   ...
                   "tryCatch": tryCatch,
                   # poie dependencies
                   "checkData": poie.datatype.checkData,
                   # Python libraries
                   "math": math,
                   }
        ...
        for cellName, cellConfig in list(engineConfig.cells.items()):
            if cellConfig.shared and cellName not in sharedState.cells:
                value = poie.datatype.jsonDecoder(cellConfig.avroType, cellConfig.initJsonNode)
        ...
        for poolName, poolConfig in list(engineConfig.pools.items()):
            if poolConfig.shared and poolName not in sharedState.pools:
                value = poie.datatype.jsonDecoder(poie.datatype.AvroMap(poolConfig.avroType), poolConfig.initJsonNode)
        ...
        for index in range(multiplicity):
            ...
            if engineConfig.method == Method.FOLD:
                zero = poie.datatype.jsonDecoder(engineConfig.output, json.loads(engineConfig.zero))
            else:
                zero = None
    ...
    def fromJson(src, options=None, version=None, sharedState=None, multiplicity=1, style="pure", debug=False):
        ...
        :return: a list of scoring engine instances
        """
        return PFAEngine.fromAst(poie.reader.jsonToAst(src), options, version, sharedState, multiplicity, style, debug)
    ...
    def fromYaml(src, options=None, version=None, sharedState=None, multiplicity=1, style="pure", debug=False):
        ...
        :return: a list of scoring engine instances
        """
        return PFAEngine.fromAst(poie.reader.yamlToAst(src), options, version, sharedState, multiplicity, style, debug)
...
class FastAvroCorrector(object):
    ...
    def correctFastAvro(self, x, avroType):
        if isinstance(avroType, (poie.datatype.AvroString, poie.datatype.AvroEnum)) and isinstance(x, str):
            return x.decode("utf-8", "replace")

        elif isinstance(avroType, (poie.datatype.AvroBytes, poie.datatype.AvroFixed)) and isinstance(x, str):
            return x.encode("utf-8", "replace")

        elif isinstance(avroType, poie.datatype.AvroArray):
            itemType = avroType.items
            for i in range(len(x)):
                x[i] = self.correctFastAvro(x[i], itemType)

        elif isinstance(avroType, poie.datatype.AvroMap):
        ...
        elif isinstance(avroType, poie.datatype.AvroRecord):
        ...
        elif isinstance(avroType, poie.datatype.AvroUnion):
            for tpe in avroType.types:
                if (isinstance(tpe, (poie.datatype.AvroString, poie.datatype.AvroEnum)) and isinstance(x, str)) or \
                   (isinstance(tpe, (poie.datatype.AvroBytes, poie.datatype.AvroFixed)) and isinstance(x, str)) or \
                   (isinstance(tpe, poie.datatype.AvroArray) and isinstance(x, (list, tuple))) or \
                   (isinstance(tpe, poie.datatype.AvroMap) and isinstance(x, dict)):
                    return self.correctFastAvro(x, tpe)

                elif (isinstance(tpe, (poie.datatype.AvroString, poie.datatype.AvroEnum)) and isinstance(x, str)) or \
                     (isinstance(tpe, (poie.datatype.AvroBytes, poie.datatype.AvroFixed)) and isinstance(x, str)):
                    return x

                elif isinstance(tpe, poie.datatype.AvroRecord) and isinstance(x, dict):
                    try:
                        return self.correctFastAvro(x, tpe)
                    except KeyError:
