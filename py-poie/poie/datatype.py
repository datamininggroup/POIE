...
def jsonToAvroType(x):
    """Convert a JSON-encoded Avro type (AVSC file) into a poie.datatype.AvroType.

    :type x: string
    :param x: JSON-encoded Avro type
    :rtype: pypoie.datatype.AvroType
...
def jsonNodeToAvroType(x):
    """Convert a Python-encoded Avro type into a poie.datatype.AvroType.
...
def schemaToAvroType(schema):
    """Convert an avro.schema into a poie.datatype.AvroType.
...
def avroTypeToSchema(avroType):
    """Convert a poie.datatype.AvroType into an avro.schema.

    :type avroType: pypoie.datatype.AvroType
...
class AvroType(Type):
    ...
    def __hash__(self):
        ...
        """TDH Changed it to a str but the number is non-unquie"""
        """TODO - use somethng other than a hash"""
    ...
    def accepts(self, other, memo=None, checkRecord=True):
        ...
        :type self: pypoie.datatype.AvroType
        :param self: the expected signature to be matched
        :type other: pypoie.datatype.AvroType
...
class AvroFixed(AvroRaw, AvroCompiled):
    ...
    def __init__(self, size, name=None, namespace=None):
        ...
        if name is None:
            name = poie.util.uniqueFixedName()
...
class AvroArray(AvroContainer):
    ...
    def __init__(self, items):
        ...
        :type items: pypoie.datatype.AvroType
...
class AvroMap(AvroContainer, AvroMapping):
    ...
    def __init__(self, values):
        ...
        :type values: pypoie.datatype.AvroType
...
class AvroRecord(AvroContainer, AvroMapping, AvroCompiled):
    ...
    def __init__(self, fields, name=None, namespace=None):
        ...
        :type fields: list of poie.datatype.AvroField
        ...
        """
        if name is None:
            name = poie.util.uniqueRecordName()
    ...
    @property
    def fields(self):
        """Get the fields as a list of poie.datatype.AvroField objects."""
        return [AvroField.fromSchema(x) for x in self.schema.fields]
    @property
    def fieldsDict(self):
        """Get the fields as a dict from field names to poie.datatype.AvroField objects."""
        return dict((x.name, x) for x in self.fields)
    def field(self, name):
        """Get one poie.datatype.AvroField object by name."""
...
class AvroUnion(AvroType):
    ...
    def __init__(self, types):
        ...
        :type types: list of poie.datatype.AvroType
        :param types: possible types for this union in the order of their resolution
        """
        names = set([x.name for x in types])
        if len(types) != len(names):
            raise poie.errors.AvroException("duplicate in union: " + ", ".join(map(ts, types)))
        if "union" in names:
            raise poie.errors.AvroException("nested union: " + ", ".join(map(ts, types)))
        self._schema = avro.schema.UnionSchema([], avro.schema.Names())
        self._schema._schemas = [x.schema for x in types]
    @property
    def types(self):
        """Get the possible types for this union as a list of poie.datatype.AvroType."""
...
class AvroField(object):
    """Field of a poie.datatype.AvroRecord."""
    @staticmethod
    ...
    @property
    def avroType(self):
        """Get the field type as a poie.datatype.AvroType."""
...
class AvroPlaceholder(object):
    ...
    @property
    def avroType(self):
        """Called after ``poie.datatype.AvroTypeBuilder`` ``resolveTypes`` to get the resolved type."""
    ...
    @property
    def parser(self):
        """The ``poie.datatype.ForwardDeclarationParser`` responsible for this placeholder."""
        return self.forwardDeclarationParser

class AvroFilledPlaceholder(AvroPlaceholder):
    """Used to create poie.datatype.AvroPlaceholder objects to satisfy functions that require them, yet the type is already known."""
...
class ForwardDeclarationParser(object):
    """Container that stores Avro types as they're collected from a PFA file, returning poie.datatype.AvroPlaceholder objects, and then resolves those types independent of the order in which they were read from the file."""
    ...
    def parse(self, jsonStrings):
        ...
        while unresolvedSize != 0:
            ...
            if unresolvedSize == lastUnresolvedSize:
                raise poie.errors.SchemaParseException("Could not resolve the following types:\n    " +
        ...
        if isinstance(description, str):
            ...
            if description == {"type": "null"}:
                ...
                if self.names.has_name(description.get("name"), description.get("namespace")):
                    ...
                    raise poie.errors.AvroException("new types, like {0}, cannot be defined with the parser.getAvroType or parser.getSchema methods (use parse)".format(json.dumps(description)))
...
class AvroTypeBuilder(object):
    """Factory that coordinates the process of collecting Avro strings, putting them in a poie.datatype.ForwardDeclarationParser, and then resolving them all at the end, independent of order."""
...
def jsonDecoder(avroType, value):
    """Decode a JSON object as a given poie.datatype.AvroType.
        ...
        if isinstance(value, dict):
            ...
            for field in avroType.fields:
                if field.name in value:
                    ...
                    raise poie.errors.AvroException("{0} does not match schema {1}".format(json.dumps(value), ts(avroType)))
            return out
...
def jsonEncoder(avroType, value, tagged=True):
    """Encode an object as JSON, given poie.datatype.AvroType.
    ...
    if isinstance(avroType, AvroNull) and value is None:
        ...
        if isinstance(value, dict) and len(value) == 1:
            ...
            for t in avroType.types:
                ...
                except poie.errors.AvroException:
                    pass
                else:
                    if tagged:
                        return {t.name: out}
...
def compare(avroType, x, y):
    ...
    if isinstance(avroType, AvroNull) and x is None and y is None:
        ...
        if isinstance(x, dict) and len(x) == 1:
            ...
            for ti, t in enumerate(avroType.types):
                try:
                    ...
                    xtype = t
            if xtypei is None:
                raise poie.errors.AvroException()
        ...
        raise poie.errors.AvroException("{0} or {1} does not match schema {2}".format(json.dumps(x), json.dumps(y), ts(avroType)))
...
def checkData(data, avroType):
    ...
    if isinstance(avroType, AvroNull):
        ...
        if isinstance(data, str):
            ...
            # not sure why this was originally ALSO comparing `data` to `str`
            # was originally throwing an error with `TypeError: expecting bytes, found b'hello'`
            return data
        else:
            raise TypeError("expecting {0}, found {1}".format(ts(avroType), data))
