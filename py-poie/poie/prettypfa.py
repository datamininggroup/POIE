...
from poie.pfaast import Subs
from poie.pfaast import validSymbolName
from poie.pfaast import validFunctionName
from poie.pfaast import Ast
from poie.pfaast import Method
from poie.pfaast import EngineConfig
from poie.pfaast import Cell
from poie.pfaast import Pool
from poie.pfaast import Argument
from poie.pfaast import Expression
from poie.pfaast import LiteralValue
from poie.pfaast import PathIndex
from poie.pfaast import ArrayIndex
from poie.pfaast import MapIndex
from poie.pfaast import RecordIndex
from poie.pfaast import HasPath
...
from poie.datatype import AvroTypeBuilder
from poie.errors import PFASyntaxException
from poie.errors import PrettyPfaException
from poie.genpy import PFAEngine
import poie.pfaast
import poie.util
from poie.util import avscToPretty
from poie.reader import jsonToAst
...
class InterpretationState(object):
    ...
    def __init__(self):
        ...
        self.avroTypeMemo = {}
        self.avroTypeAlias = {}
        self.functionNames = set(poie.pfaast.FunctionTable.blank().functions)
        self.cellNames = set()
        self.poolNames = set()
    ...
    def asType(self, state):
        ...
        if self.name == "array":
            ...
            if len(self.args) == 2:
                ...
                namespace, name = None, poie.util.uniqueFixedName()
            ...
            if len(self.args) == 2:
                ...
                namespace, name = None, poie.util.uniqueEnumName()
            ...
            if len(others) == 0:
                namespace, name = None, poie.util.uniqueRecordName()
            elif len(others) == 1:
...
class Parser(object):
    ...
    def __init__(self, wholeDocument):
        ...
        :param wholeDocument: if ``True``, this parser expects a whole PFA document and ``parse`` returns a poie.pfaast.EngineConfig; otherwise, this parser expects a PFA expression and ``parse`` returns a poie.pfaast.Expression
        """
        ...
        if self.wholeDocument:
            def p_document(p):
                ...
                if "name" in keys:
                    ...
                    _name = poie.util.uniqueEngineName()
    ...
    def parse(self, text, subs):
        ...
        :rtype: pypoie.pfaast.EngineConfig or pypoie.pfaast.Expression
...
def subs(originalAst, **subs2):
    """Apply substitutions to all poie.pfaast.Subs nodes in a PFA abstract syntax tree using its ``replace`` method.

    :type originalAst: pypoie.pfaast.Ast
    :param originalAst: abstract syntax tree to replace
    :type subs2: dict from substitution names to substitutions
    :param subs2: replacement values as PFA poie.pfaast.Ast, PrettyPFA strings, or PFA Pythonized JSON
    :rtype: pypoie.pfaast.Ast
...
def ast(text, check=True, version=None, subs={}, **subs2):
    ...
    :param version: version of the PFA language to use while interpreting (``None`` defaults to poie.version.defaultPFAVersion)
    :type subs: dict from substitution names to substitutions
    :param subs: replacement values as PFA poie.pfaast.Ast, PrettyPFA strings, or PFA Pythonized JSON
    :type subs2: dict from substitution names to substitutions
    :param subs2: added to ``subs`` (a more convenient way to pass them)
    :rtype: pypoie.pfaast.EngineConfig
...
def engine(text, options=None, version=None, sharedState=None, multiplicity=1, style="pure", debug=False, subs={}, **subs2):
    ...
    :type subs2: dict from substitution names to substitutions
    :param subs2: added to ``subs`` (a more convenient way to pass them)
    :rtype: list of poie.genpy.PFAEngine
...
def ppfas(text, subs={}, **subs2):
    ...
    :type subs2: dict from substitution names to substitutions
    :param subs2: added to ``subs`` (a more convenient way to pass them)
    :rtype: list of poie.pfaast.Expression
...
def ppfa(text, subs={}, **subs2):
    ...
    :type subs2: dict from substitution names to substitutions
    :param subs2: added to ``subs`` (a more convenient way to pass them)
    :rtype: pypoie.pfaast.Expression
