...
def ts(avroType):
    ...
    Note: if poie.util.TYPE_ERRORS_IN_PRETTYPFA is ``True``, the type will be printed in PrettyPFA notation; otherwise, as a raw Avro schema.
...
def uniqueEngineName():
    """Provide an engine name, incrementing poie.util.uniqueEngineNameCounter to ensure uniqueness of values supplied by this function."""
    sys.modules["poie.util"].uniqueEngineNameCounter += 1
    return "Engine_{0}".format(sys.modules["poie.util"].uniqueEngineNameCounter)

uniqueRecordNameCounter = 0
def uniqueRecordName():
    """Provide a record type name, incrementing poie.util.uniqueRecordNameCounter to ensure uniqueness of values supplied by this function."""
    sys.modules["poie.util"].uniqueRecordNameCounter += 1
    return "Record_{0}".format(sys.modules["poie.util"].uniqueRecordNameCounter)

uniqueEnumNameCounter = 0
def uniqueEnumName():
    """Provide an enum type name, incrementing poie.util.uniqueEnumNameCounter to ensure uniqueness of values supplied by this function."""
    sys.modules["poie.util"].uniqueEnumNameCounter += 1
    return "Enum_{0}".format(sys.modules["poie.util"].uniqueEnumNameCounter)

uniqueFixedNameCounter = 0
def uniqueFixedName():
    """Provide a fixed type name, incrementing poie.util.uniqueFixedNameCounter to ensure uniqueness of values supplied by this function."""
    sys.modules["poie.util"].uniqueFixedNameCounter += 1
    return "Fixed_{0}".format(sys.modules["poie.util"].uniqueFixedNameCounter)
...
class DynamicScope(object):
    ...
    def __init__(self, parent):
        """:type parent: pypoie.util.DynamicScope or ``None``
