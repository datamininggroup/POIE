...
import poie.lib.core
import poie.lib.pfamath
import poie.lib.spec
import poie.lib.link
import poie.lib.kernel
import poie.lib.la
import poie.lib.metric
import poie.lib.rand
import poie.lib.pfastring
import poie.lib.regex
import poie.lib.parse
import poie.lib.cast
import poie.lib.array
import poie.lib.map
import poie.lib.bytes
import poie.lib.fixed
import poie.lib.enum
import poie.lib.pfatime
import poie.lib.impute
import poie.lib.interp
import poie.lib.prob.dist
import poie.lib.stat.pfatest
import poie.lib.stat.sample
import poie.lib.stat.change
import poie.lib.model.reg
import poie.lib.model.tree
import poie.lib.model.cluster
import poie.lib.model.neighbor
import poie.lib.model.naive
import poie.lib.model.neural
import poie.lib.model.svm
...
from poie.signature import IncompatibleTypes
from poie.signature import LabelData
from poie.signature import Sig
from poie.signature import PFAVersion
from poie.datatype import *
import poie.options
from poie.util import ts
...
def inferType(expr, symbols=None, cells=None, pools=None, fcns=None, version=PFAVersion(0, 0, 0)):
    ...
    :type expr: pypoie.pfaast.Expression
    :param expr: expression to examine
    :type symbols: dict from symbol name to poie.datatype.AvroType
    :param symbols: data types of variables used in the expression
    :type cells: dict from cell name to poie.datatype.AvroType
    :param cells: data types of cells used in the expression
    :type pools: dict from pool name to poie.datatype.AvroType
    :param pools: data types of pools used in the expression
    :type fcns: dict from function name to poie.fcn.Fcn
    ...
    context, result = expr.walk(NoTask(), symbolTable, functionTable, poie.options.EngineOptions(None, None), version)
...
class SymbolTable(object):
    ...
    def __init__(self, parent, symbols, cells, pools, sealedAbove, sealedWithin):
        """:type parent: pypoie.pfaast.SymbolTable or ``None``
    ...
    def getLocal(self, name):
        ...
        :rtype: poie.datatype.AvroType or ``None``
    ...
    def put(self, name, avroType):
        ...
        :type name: string
        :param name: name of the symbol
        :type avroType: poie.datatype.AvroType
    ...
    def newScope(self, sealedAbove, sealedWithin):
        ...
        :type sealedWithin: bool
        :param sealedWithin: if ``True``, new symbols cannot be created in this scope
        :rtype: pypoie.pfaast.SymbolTable
    ...
    def inThisScope(self):
        ...
        :rtype: dict from symbol name to poie.datatype.AvroType
...
class UserFcn(Fcn):
    ...
    def __init__(self, name, sig):
        ...
        :type sig: pypoie.signature.Sig
        :param sig: function signature (note that poie.signature.Sigs is not allowed for user-defined functions)
        """
        self.name = name
        self.sig = sig
    ...
    def fromFcnDef(n, fcnDef):
        ...
        :type fcnDef: pypoie.pfaast.FcnDef
        :param fcnDef: the abstract syntax tree function definition
        :rtype: pypoie.pfaast.UserFcn
...
class EmitFcn(Fcn):
    ...
    def __init__(self, outputType):
        """:type outputType: poie.datatype.AvroType
...
class FunctionTable(object):
    ...
    def __init__(self, functions):
        """:type functions: dict from function name to poie.fcn.Fcn
    ...
    def blank():
        ...
        functions = {}
        functions.update(poie.lib.core.provides)
        functions.update(poie.lib.pfamath.provides)
        functions.update(poie.lib.spec.provides)
        functions.update(poie.lib.link.provides)
        functions.update(poie.lib.kernel.provides)
        functions.update(poie.lib.la.provides)
        functions.update(poie.lib.metric.provides)
        functions.update(poie.lib.rand.provides)
        functions.update(poie.lib.pfastring.provides)
        functions.update(poie.lib.regex.provides)
        functions.update(poie.lib.parse.provides)
        functions.update(poie.lib.cast.provides)
        functions.update(poie.lib.array.provides)
        functions.update(poie.lib.map.provides)
        functions.update(poie.lib.bytes.provides)
        functions.update(poie.lib.fixed.provides)
        functions.update(poie.lib.enum.provides)
        functions.update(poie.lib.pfatime.provides)
        functions.update(poie.lib.impute.provides)
        functions.update(poie.lib.interp.provides)
        functions.update(poie.lib.prob.dist.provides)
        functions.update(poie.lib.stat.pfatest.provides)
        functions.update(poie.lib.stat.sample.provides)
        functions.update(poie.lib.stat.change.provides)
        functions.update(poie.lib.model.reg.provides)
        functions.update(poie.lib.model.tree.provides)
        functions.update(poie.lib.model.cluster.provides)
        functions.update(poie.lib.model.neighbor.provides)
        functions.update(poie.lib.model.naive.provides)
        functions.update(poie.lib.model.neural.provides)
        functions.update(poie.lib.model.svm.provides)

        # TODO: functions.update(poie.lib.other.provides)...
...
class AstContext(object):
    """Trait for poie.pfaast.Ast context classes."""
    pass

class ArgumentContext(AstContext):
    """Subtrait for context classes of poie.pfaast.Argument."""
    def calls(self):
        raise NotImplementedError

class ExpressionContext(ArgumentContext):
    """Subtrait for context classes of poie.pfaast.Expression."""
    def retType(self):
        raise NotImplementedError

class FcnContext(ArgumentContext):
    """Subtrait for context classes of poie.pfaast.FcnDef."""
    def fcnType(self):
        raise NotImplementedError

class TaskResult(object):
    """Trait for result of a generic task, passed to poie.pfaast.Ast ``walk``."""
    pass

class Task(object):
    """Trait for a generic task, passed to poie.pfaast.Ast ``walk``."""
    def __call__(self, astContext, engineOptions, resoledType=None):
        """Perform a task from context generated by a poie.pfaast.Ast ``walk``.

        :type astContext: pypoie.pfaast.AstContext
        :param astContext: data about the poie.pfaast.Ast node after type-checking
        :type engineOptions: pypoie.options.EngineOptions
        :param engineOptions: implementation options
        :type resolvedType: poie.datatype.AvroType or ``None``
        :param resolvedType: ?
        :rtype: pypoie.pfaast.TaskResult
        :return: the result of this task
        """
        raise NotImplementedError

class NoTask(Task):
    """Concrete poie.pfaast.Task that does nothing, used for type-checking without producing an engine."""

    class EmptyResult(TaskResult):
        """Concrete poie.pfaast.TaskResult that contains no result."""
        pass
    ...
    def __call__(self, astContext, engineOptions, resolvedType=None):
        ...
        :type resolvedType: poie.datatype.AvroType or ``None``
        :param resolvedType: ?
        :rtype: pypoie.pfaast.NoTask.EmptyResult
        :return: empty result object
        """
        return self.EmptyResult()

def check(engineConfig, version):
    """Check a PFA abstract syntax tree for semantic errors using a poie.pfaast.NoTask to avoid unnecessary work.

    Raises an exception if the document is not valid.

    :type engineConfig: pypoie.pfaast.EngineConfig
    :param engineConfig: abstract syntax tree for a complete PFA document
    :type version: pypoie.signature.PFAVersion
    ...
    engineConfig.walk(NoTask(),
                      SymbolTable.blank(),
                      FunctionTable.blank(),
                      poie.options.EngineOptions(engineConfig.options, None),
                      version)
...
class Ast(object):
    ...
    def collect(self, pf):
        ...
        :param pf: partial function that takes any poie.pfaast.Ast as an argument, returning anything
        :type pf.isDefinedAt: callable
        :param pf.isDefinedAt: domain that takes any poie.pfaast.Ast as an argument, returning ``True`` if this item is in the domain, ``False`` otherwise
    ...
    def replace(self, pf):
        ...
        :type pf: callable with ``isDefinedAt`` method
        :param pf: partial function that takes any poie.pfaast.Ast as an argument, returning a replacement poie.pfaast.Ast
        :type pf.isDefinedAt: callable
        :param pf.isDefinedAt: domain that takes any poie.pfaast.Ast as an argument, returning ``True`` if this item is in the domain, ``False`` otherwise
        :rtype: new poie.pfaast.Ast tree
    ...
    def walk(self, task, version):
        """Walk over tree applying a poie.pfaast.Task while checking for semantic errors.

        This is how Python is generated from an abstract syntax tree: the poie.pfaast.Task in that case is poie.genpy.GeneratePython.

        :type task: pypoie.pfaast.Task
        :param task: task to perform
        :type version: pypoie.signature.PFAVersion
        :param version: PFA language version in which to interpret this PFA
        """
        self.walk(task, SymbolTable.blank(), FunctionTable.blank(), poie.options.EngineOptions(None, None), version)
...
class EngineConfig(Ast):
    ...
    @property
    def input(self):
        """Input type after type resolution (poie.datatype.AvroType)."""
        return self.inputPlaceholder.avroType

    @property
    def output(self):
        """Output type after type resolution (poie.datatype.AvroType)."""
    ...
    def collect(self, pf):
        ...
        return super(EngineConfig, self).collect(pf) + \
               poie.util.flatten(x.collect(pf) for x in self.begin) + \
               poie.util.flatten(x.collect(pf) for x in self.action) + \
               poie.util.flatten(x.collect(pf) for x in self.end) + \
               poie.util.flatten(x.collect(pf) for x in list(self.fcns.values())) + \
               poie.util.flatten(x.collect(pf) for x in list(self.cells.values())) + \
               poie.util.flatten(x.collect(pf) for x in list(self.pools.values()))
    ...
    def walk(self, task, symbolTable, functionTable, engineOptions, pfaVersion):
        ...
        beginCalls = set(poie.util.flatten([x[0].calls for x in beginContextResults]))
        ...
        if self.merge is not None:
            ...
            mergeCalls = set(poie.util.flatten([x[0].calls for x in mergeContextResults]))
        ...
        endCalls = set(poie.util.flatten([x[0].calls for x in endContextResults]))
        ...
        actionCalls = set(poie.util.flatten([x[0].calls for x in actionContextResults]))
...
class Cell(Ast):
    ...
    def __init__(self, avroPlaceholder, init, shared, rollback, source, pos=None):
        """:type avroPlaceholder: poie.datatype.AvroPlaceholder
        ...
        :type shared: bool
        :param shared: if ``True``, this cell shares data with all others in the same poie.genpy.SharedState
        :type rollback: bool
        :param rollback: if ``True``, this cell rolls back its value when it encounters an exception
        :type source: poie.pfaast.CellPoolSource string
...
class Pool(Ast):
    ...
    def __init__(self, avroPlaceholder, init, shared, rollback, source, pos=None):
        ...
        :param init: serialized JSON string containing initial data or a function that produces it (from an external file, usually)
        :type shared: bool
        :param shared: if ``True``, this pool shares data with all others in the same poie.genpy.SharedState
        :type rollback: bool
...
class ArrayIndex(PathIndex):
    """Array indexes, which are concrete poie.pfaast.PathIndex elements that dereference arrays (expressions of ``int`` type)."""
    def __init__(self, i, t): pass

@poie.util.case
class MapIndex(PathIndex):
    """Map indexes, which are concrete poie.pfaast.PathIndex elements that dereference maps (expressions of ``string`` type)."""
    def __init__(self, k, t): pass

@poie.util.case
class RecordIndex(PathIndex):
    """Record indexes, which are concrete poie.pfaast.PathIndex elements that dereference records (literal ``string`` expressions)."""
    def __init__(self, f, t): pass

class HasPath(object):
    """Mixin for poie.pfaast.Ast classes that have paths (``attr``, ``cell``, ``pool``)."""
    ...
    def walkPath(self, avroType, task, symbolTable, functionTable, engineOptions, version):
        ...
        :type version: pypoie.signature.PFAVersion
        :param version: version of the PFA language in which to interpret this PFA
        :rtype: (poie.datatype.AvroType, set of strings, list of poie.pfaast.PathIndex)
...
class FcnDef(Argument):
    ...
    @property
    def params(self):
        """Resolved parameter types (list of {string: poie.datatype.AvroType} singletons)."""
        return [{list(t.keys())[0]: list(t.values())[0].avroType} for t in self.paramsPlaceholder]

    @property
    def paramsDict(self):
        """Resolved parameter types as an unordered dictionary (dict of poie.datatype.AvroType)."""
        return dict((list(t.keys())[0], list(t.values())[0].avroType) for t in self.paramsPlaceholder)

    @property
    def ret(self):
        """Resolved return type (poie.datatype.AvroType)."""
    ...
    def collect(self, pf):
        ...
        return super(FcnDef, self).collect(pf) + \
               poie.util.flatten(x.collect(pf) for x in self.body)
    ...
    def walk(self, task, symbolTable, functionTable, engineOptions, version):
        ...
        context = self.Context(FcnType([list(t.values())[0] for t in self.params], self.ret), set(poie.util.flatten([x[0].calls for x in results])), self.paramNames, self.paramsDict, self.ret, scope.inThisScope, [x[1] for x in results])
        return context, task(context, engineOptions)
...
class FcnRefFill(Argument):
    ...
    def __init__(self, name, fill, pos=None):
        ...
        :type fill: dict from parameter names to poie.pfaast.Argument
    ...
    def collect(self, pf):
        ...
        return super(FcnRefFill, self).collect(pf) + \
               poie.util.flatten(x.collect(pf) for x in list(self.fill.values()))
...
class CallUserFcn(Expression):
    ...
    def collect(self, pf):
        ...
        return super(CallUserFcn, self).collect(pf) + \
               ...
               poie.util.flatten(x.collect(pf) for x in self.args)
...
class Literal(LiteralValue):
    ...
    @property
    def avroType(self):
        """Resolved data type (poie.datatype.AvroType)."""
        return self.avroPlaceholder.avroType
...
class NewObject(Expression):
    ...
    def __init__(self, fields, avroPlaceholder, pos=None):
        """:type fields: dict from field names to poie.pfaast.Expression
    ...
    @property
    def avroType(self):
        """Resolved map or record type (poie.datatype.AvroType)."""
        return self.avroPlaceholder.avroType
    ...
    def collect(self, pf):
        ...
        return super(NewObject, self).collect(pf) + \
               poie.util.flatten(x.collect(pf) for x in list(self.fields.values()))
...
class NewArray(Expression):
    ...
    def __init__(self, items, avroPlaceholder, pos=None):
        """:type items: list of poie.pfaast.Expression
    ...
    @property
    def avroType(self):
        """Resolved array type (poie.datatype.AvroType)."""
        return self.avroPlaceholder.avroType
    ...
    def collect(self, pf):
        ...
        return super(NewArray, self).collect(pf) + \
               poie.util.flatten(x.collect(pf) for x in self.items)
...
class Do(Expression):
    ...
    def walk(self, task, symbolTable, functionTable, engineOptions, version):
        ...
        context = self.Context(inferredType, set(poie.util.flatten([x[0].calls for x in results])).union(set([self.desc])), scope.inThisScope, [x[1] for x in results])
        return context, task(context, engineOptions)
...
class Let(Expression):
    ...
    def __init__(self, values, pos=None):
        """:type values: dict from variable (symbol) name to poie.pfaast.Expression
    ...
    def collect(self, pf):
        ...
        return super(Let, self).collect(pf) + \
               poie.util.flatten(x.collect(pf) for x in list(self.values.values()))
...
class AttrGet(Expression, HasPath):
    ...
    def __init__(self, expr, path, pos=None):
        ...
        :type path: list of poie.pfaast.Expression
    ...
    def collect(self, pf):
        ...
        return super(AttrGet, self).collect(pf) + \
               ...
               poie.util.flatten(x.collect(pf) for x in self.path)
...
class AttrTo(Expression, HasPath):
    ...
    def __init__(self, expr, path, to, pos=None):
        ...
        :type path: list of poie.pfaast.Expression
        :param path: array, map, and record indexes to extract from the base object
        :type to: pypoie.pfaast.Argument
...
class PoolTo(Expression, HasPath):
    ...
    def __init__(self, pool, path, to, init, pos=None):
        ...
        :type to: pypoie.pfaast.Argument
        :param to: expression for a replacement object or function reference to use as an updator
        :type init: pypoie.pfaast.Expression
...
class PoolDel(Expression):
    ...
    def __init__(self, pool, dell, pos=None):
        """:type pool: string
        :param pool: pool name
        :type dell: pypoie.pfaast.Expression
...
class If(Expression):
    ...
    def __init__(self, predicate, thenClause, elseClause, pos=None):
        """:type predicate: pypoie.pfaast.Expression
        :param predicate: test expression that evaluates to ``boolean``
        :type thenClause: list of poie.pfaast.Expression
        :param thenClause: expressions to evaluate if the ``predicate`` evaluates to ``true``
        :type elseClause: list of poie.pfaast.Expression or ``None``
    ...
    def collect(self, pf):
        ...
        return super(If, self).collect(pf) + \
               ...
               poie.util.flatten(x.collect(pf) for x in self.thenClause) + \
               (poie.util.flatten(x.collect(pf) for x in self.elseClause) if self.elseClause is not None else [])
...
class Cond(Expression):
    ...
    def __init__(self, ifthens, elseClause, pos=None):
        """:type ifthens: list of poie.pfaast.If
    ...
    def collect(self, pf):
        ...
        return super(Cond, self).collect(pf) + \
               poie.util.flatten(x.collect(pf) for x in self.ifthens) + \
               (poie.util.flatten(x.collect(pf) for x in self.elseClause) if self.elseClause is not None else [])
...
class For(Expression):
    ...
    def __init__(self, init, predicate, step, body, pos=None):
        """:type init: dict from new variable names to poie.pfaast.Expression
        :param init: initial values for the dummy variables
        :type predicate: pypoie.pfaast.Expression
        :param predicate: test expression that evaluates to ``boolean``
        :type step: dict from variable names to poie.pfaast.Expression
    ...
    def collect(self, pf):
        ...
        return super(For, self).collect(pf) + \
               poie.util.flatten(x.collect(pf) for x in list(self.init.values())) + \
               self.predicate.collect(pf) + \
               poie.util.flatten(x.collect(pf) for x in list(self.step.values())) + \
               poie.util.flatten(x.collect(pf) for x in self.body)
...
class Foreach(Expression):
    ...
    def __init__(self, name, array, body, seq, pos=None):
        ...
        :type array: pypoie.pfaast.Expression
        :param array: expression that evaluates to an array
        :type body: list of poie.pfaast.Expression
        :param body: expressions to evaluate for each item of the array
        :type seq: bool
        :param seq: if ``False``, seal the scope from above and allow implementations to parallelize (poie doesn't)
        :type pos: string or ``None``
        :param pos: source file location from the locator mark
        """
...
class Forkeyval(Expression):
    ...
    def __init__(self, forkey, forval, map, body, pos=None):
        ...
        :type map: pypoie.pfaast.Expression
...
class CastCase(Ast):
    ...
    @property
    def avroType(self):
        """Resolved subtype (poie.datatype.AvroType)."""
        return self.avroPlaceholder.avroType
    ...
    def walk(self, task, symbolTable, functionTable, engineOptions, version):
        ...
        results = [x.walk(task, scope, functionTable, engineOptions, version) for x in self.body]
        context = self.Context(results[-1][0].retType, self.named, self.avroType, set(poie.util.flatten([x[0].calls for x in results])), scope.inThisScope, [x[1] for x in results])
        return context, task(context, engineOptions)
...
class CastBlock(Expression):
    ...
    def __init__(self, expr, castCases, partial, pos=None):
        ...
        :type castCases: list of poie.pfaast.CastCase
    ...
    def collect(self, pf):
        ...
        return super(CastBlock, self).collect(pf) + \
               self.expr.collect(pf) + \
               poie.util.flatten(x.collect(pf) for x in self.castCases)
...
class Upcast(Expression):
    ...
    @property
    def avroType(self):
        """Resolved supertype (poie.datatype.AvroType)."""
        return self.avroPlaceholder.avroType
...
class IfNotNull(Expression):
    ...
    def __init__(self, exprs, thenClause, elseClause, pos=None):
        """:type exprs: dict from new variable names to poie.pfaast.Expression
    ...
    def collect(self, pf):
        ...
        return super(IfNotNull, self).collect(pf) + \
               poie.util.flatten(x.collect(pf) for x in list(self.exprs.values())) + \
               poie.util.flatten(x.collect(pf) for x in self.thenClause) + \
               (poie.util.flatten(x.collect(pf) for x in self.elseClause) if self.elseClause is not None else [])
...
class BinaryFormatter(object):
    ...
    def formatToDeclare(value, f, pos, output):
        """Convert a format string to a poie.pfaast.BinaryFormatter.Declare object.

        :type value: pypoie.pfaast.TaskResult
        ...
        :rtype: poie.pfaast.BinaryFormatter.Declare subclass
...
class Pack(Expression):
    ...
    def __init__(self, exprs, pos=None):
        """:type exprs: list of (string, poie.pfaast.Expression)
    ...
    def collect(self, pf):
        ...
        return super(Pack, self).collect(pf) + \
               poie.util.flatten(v.collect(pf) for k, v in self.exprs)
...
class Unpack(Expression):
    ...
    def __init__(self, bytes, format, thenClause, elseClause, pos=None):
        """:type bytes: pypoie.pfaast.Expression
...
class Try(Expression):
    ...
    def __init__(self, exprs, filter, pos=None):
        """:type exprs: list of poie.pfaast.Expression
    ...
    def collect(self, pf):
        ...
        return super(Try, self).collect(pf) + \
               poie.util.flatten(x.collect(pf) for x in self.exprs)
    ...
    def walk(self, task, symbolTable, functionTable, engineOptions, version):
        ...
        context = self.Context(inferredType, set(poie.util.flatten([x[0].calls for x in results])).union(set([self.desc])), scope.inThisScope, [x[1] for x in results], exprType, self.filter)
        return context, task(context, engineOptions)
...
class Log(Expression):
    ...
    def walk(self, task, symbolTable, functionTable, engineOptions, version):
        ...
        scope = symbolTable.newScope(True, True)
        results = [x.walk(task, scope, functionTable, engineOptions, version) for x in self.exprs]
        context = self.Context(AvroNull(), set(poie.util.flatten([x[0].calls for x in results])).union(set([self.desc])), scope.inThisScope, results, self.namespace)
        return context, task(context, engineOptions)
