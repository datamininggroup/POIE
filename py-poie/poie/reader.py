...
def jsonToAst(jsonInput):
    ...
    """
    """ TDH changed but does not cover all use cases """
...
def jsonToFcnDefs(jsonInput, where=""):
    ...
    :rtype: dict of poie.pfaast.FcnDef
    ...
    if isinstance(data, dict):
        ...
        if ("type" not in keys) or ("init" not in keys) or (not keys.issubset(set(["type", "init", "shared", "rollback", "source"]))):
            ...
            if _source == "avro":
                ...
                # def getit(avroType):
                #     return poie.datatype.jsonDecoder(avroType, value)
                # _init = getit
